...
renderSidebar({ role: 'admin', activePath: 'teachers.html', user: s.user });

const tchTable = requireElement<HTMLElement>('#tch-table');

async function loadUserList() {
  try {
    const res = await fetch(`${API_URL}/admin/users?role=teacher`, { headers: authOnly() });
    if (res.status === 401) { window.location.href = '../login.html'; return; }
    const data = await readJson<{ users: AnyUser[] }>(res);
    if (!data) return;

    if (!data.users.length) { tchTable.innerHTML = `<tr><td colspan="3" style="text-align:center;padding:40px">暂无教师</td></tr>`; return; }
  ...
  } catch { tchTable.innerHTML = '<tr><td colspan="3" style="text-align:center;padding:40px;color:var(--danger)">加载失败</td></tr>'; }
}

tchTable.addEventListener('click', (e: Event) => {
